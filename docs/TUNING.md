...
| `-rtr`  | **off** | Repacks CPU-resident experts at load into the row-interleaved `_R4`/`_R8` layout the AVX2/VNNI kernels prefer. What that really chooses is the *processor*: the repacked types have no CUDA kernel (they exist only under `ggml/src/iqk/`), so the CPU must run the expert GEMM itself — whereas with it off the scheduler streams the activated experts to the GPU for each micro-batch (the path `-ooae` trims). Which side wins depends on `-ub`. **Measured here (ncmoe=22, llama-bench's default `-ub 512`): prefill +16% (208→242 t/s), generation unchanged** — at a micro-batch that small the PCIe transfer is not amortised and the CPU is ahead. At `-ub 4096` on DeepSeek-V4-Flash the same flag turned 1376 pp into 478 (2.9× slower, RESULTS §21); it is a regime, not a property. Generation is unaffected either way — bandwidth-bound, and reorganising the bytes doesn't reduce how many are read per token. It forces `--no-mmap`, so every start re-reads the whole model (~114 GiB Q4 / ~195 GiB Q8) from disk. Worth trying only with a small `-ub`, a narrow PCIe link, or a CPU that beats the GPU at quantised GEMM — and only measured (`./bench.sh rtr` compares both at the profile's `-ub`). Do **not** bake it into the file with `llama-quantize --repack`: a repacked GGUF pins the experts to the CPU permanently, `-rtr` off or not — the `step-3.7-flash-q4-r4` profile is that trap sprung, and its model file was deleted (TODO #8). |
