...
## Carried forward — 146 lines

| patch | what it does |
|---|---|
| `keep-build_deepseek4.patch` | **the fix.** The DSV4 raw-branch SWA window view derives its start from the 256-padded KV length while the mask rows are indexed from `kv_head`; when the padding slack exceeds the window slack the view starts past what the leading rows need and they arrive at `FLASH_ATTN_EXT` entirely `-inf`, which becomes NaN. Clamps the start to the oldest cell row 0 still needs. RESULTS §49.2 |
